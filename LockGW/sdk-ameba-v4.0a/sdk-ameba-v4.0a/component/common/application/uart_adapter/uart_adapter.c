...
int myUartadapter_init()
{
	...
	if(initFlag == 1)
	{
		...
		if(xTaskCreate(uartadapter_uart_rx_thread, ((const char*)"uart_rx"), 1024, (void *)ua_global_socket, UA_UART_THREAD_PRIORITY, NULL) != pdPASS)
			ua_printf(UA_ERROR, "%s xTaskCreate(uart_rx) failed", __FUNCTION__);
...
void userUartPrintf(char *data, int data_len)
{
    ...
    //uartadapter_uart_write_bytes(ua_global_socket,data,data_len);
}
//end levi
